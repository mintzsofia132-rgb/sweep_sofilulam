...
MACRO_DATES = {
    # --- FOMC (Federal Reserve, confirmadas) ---
    "2026-07-29",   # FOMC decisión
    "2026-09-16",   # FOMC decisión (dot plot)
    "2026-10-28",   # FOMC decisión
    "2026-12-09",   # FOMC decisión
    # --- CPI (8:30 ET, verificar cada mes en bls.gov/schedule) ---
...
def compute_levels(df5: pd.DataFrame, today: date) -> dict:
    ...
    # filtrar mechas de premarket con volumen ínfimo (ticks basura de yfinance):
    #   1) descartar velas con volumen < 10% de la mediana overnight
    #   2) recortar el rango a los percentiles 1-99 para eliminar outliers residuales
    if len(on):
        vmed = on["volume"].median()
        on = on[on["volume"] >= max(vmed * 0.10, 1)]
    if len(on):
        onh = float(on["high"].quantile(0.99))
        onl = float(on["low"].quantile(0.01))
    else:
        onh = onl = None
